...
class HttpTransport:
    ...
    __slots__ = ("_base_url", "_client", "_max_response_bytes", "_options", "_owns_client")
    _client: Any
        ...
        *,
        ...
        if body is not None:
            try:
                ...
                raise XenoteerError("invalid_request", "request is not valid JSON") from None
            if len(encoded) > self._max_response_bytes:
                raise XenoteerError("invalid_request", "request body exceeds SDK limit")
    ...
    async def upload_artifact_stream(
        ...
        *,
        ...
        try:
            async with self._client.stream(
                "POST", path, headers=headers, content=checked_chunks()
            ) as response:
                content = await _collect_bounded(response, self._max_response_bytes)
    ...
    async def delete_artifact(self, path: str) -> None:
        ...
        try:
            async with self._client.stream("DELETE", path, headers=headers) as response:
                content = await _collect_bounded(response, self._max_response_bytes)
